...
- **Progress:** The standard-library client implements bounded authenticated `/health`, `/props`, and `/v1/chat/completions` requests with strict JSON schema and local semantic validation. QNX testing exposed timeout and invalid-response behavior; the request now disables reasoning, bounds generated citations/rationales, seeds sampling, and distinguishes token-limit truncation. A captured live response then showed that the deployed server wraps otherwise valid structured JSON in one Markdown JSON fence. The client accepts only that complete outer wrapper before applying the unchanged strict parser and semantic validator. A later live response was still invalid, so parser errors now name the failing response stage and an explicitly enabled diagnostic path can capture the bounded raw server envelope without the credential. The launcher uses the requested Qwen/BLAS/CPU/context/bind settings, computes and records the first model hash, and rejects later mismatches. The CLI loads one gitignored mode-0600 `ai-llama-default.env` containing the localhost endpoint, absolute QNX paths, credential, and hash, with environment overrides. CLI, rocket fixtures, upload packaging, and user procedure are implemented. A rebuilt QNX client, diagnostic capture, and successful live finding remain required.
