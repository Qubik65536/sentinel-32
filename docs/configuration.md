...
Configuration handling is implemented for the advisory checker CLI and remains planned for the broader runtime. `sentinel-app ai-health` and `ai-check` load the gitignored, mode-0600 `config/ai-llama-default.env`; process environment values override it. `S32_AI_CONFIG_PATH` selects a different file. The repository tracks only `config/ai-llama-default.env.example`. Scenario and VM commands continue to use explicit arguments. Provider configuration cannot alter compiled scenario semantics.

Advisory precedence is process environment, then the local default file. The parser is bounded, rejects unknown or duplicate keys and malformed lines, and does not execute file contents. Safety-relevant ambiguity, invalid values, unknown enum variants, and numeric overflow fail closed. Scenario-specific channel definitions, pressure bands, timeouts, and safe states belong in the compiled scenario and cannot be overridden by process environment.
...
| `S32_LLAMA_BASE_URL` | Authenticated `llama.cpp` endpoint | QNX default is `http://127.0.0.1:8080`; never use the `0.0.0.0` bind address as a client URL |
...
Never commit the local default file, API keys, or downloaded model weights. The launcher reads the mode-0600 default file, computes the selected GGUF SHA-256, verifies a previously recorded digest when present, and atomically updates the digest in the same file. Every finding records the backend, server build, model identity/hash, prompt-contract version, written-rule hash, and snapshot identity without authorization headers or unnecessary prompt content. See `docs/ai-user-guide.md` for setup and checks.
